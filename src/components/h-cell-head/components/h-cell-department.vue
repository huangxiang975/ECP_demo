  ...
  <div>
    <div class='h-cell-wrapper'
      ...
      <div class="h-cell-tt ellipsis">
        <span>{{cellData.formfield.labelname|| 部门}}</span>
        <span class="star"
              v-if="isStar">*</span>
      </div>
    ...
    <h-department-tree v-if="dialogShow"
                       :isDialogShow.sync="dialogShow"
                       :id="cellData.datatype"
                       @ReturnDepartmentTree="returnDialog"></h-department-tree>
  </div>
</template>
...
<script lang='ts'>
  ...
  import hDepartmentTree from "@/components/components/departmentTree/index.vue";
